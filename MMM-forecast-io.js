...
//========== sunrise/sunset
    if (this.config.showSunrise) {
      ...
      if (today.sunriseTime*1000 < now && today.sunsetTime*1000 > now) {
      	var sunset = new moment.unix(today.sunsetTime).format( "HH:mm" );
   	    sunString = '<span class="wi wi-sunset xdimmed"></span> '  + sunset;
      } else {
    	var sunrise = new moment.unix(today.sunriseTime).format( "HH:mm" );
...
// ===== 6hr tick lines
  ...
  renderForecastRow: function (data, min, max) {
   ...
   // style for "-" in some min. temp. 
    if (rowMinTemp >= 0) {
      minTemp.style.paddingLeft = "10px";
    }
    ...
    */
    ...
    forecastBar.appendChild(spacer);
